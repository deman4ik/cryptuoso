...
import {
    RobotSettings as RobotSettingsRS,
    RobotState as RobotStateRS,
    T2TrendFriendRobot,
...
export interface BacktesterPositionState extends PositionState {
    backtestId: string;
    robotId: string;
}
...
export class Backtester {
    ...
    initRobots() {
        for (const [id, robot] of Object.entries(this.#robots)) {
            try {
                ...
                this.#robots[id].instance = new T2TrendFriendRobot(
                    {
                        exchange: this.#exchange,
                        timeframe: this.#timeframe,
                        strategySettings: {
                    ...
                    {
                        sma1: +robot.strategySettings["sma1"],
                        sma2: +robot.strategySettings["sma2"],
                        sma3: +robot.strategySettings["sma3"],
                        minBarsToHold: +robot.strategySettings["minBarsToHold"]
                    },
                    {
    ...
    handleHistoryCandles(candles: Candle[]) {
        Object.keys(this.#robots).forEach((id) => {
            this.#robots[id].instance.handleCandles(candles);
        });
    }
    ...
    #saveSignals = (id: string) => {
        if (this.#settings.saveSignals || this.#settings.populateHistory) {
            ...
            robot.data.alerts = [
                ...robot.data.alerts,
                ...robot.instance.robotState.alerts.map((data) => ({
                    ...data,
                    robotId: id,
                    backtestId: this.#id
                }))
            ...
            robot.data.trades = [
                ...robot.data.trades,
                ...robot.instance.robotState.trades.map((data) => ({
                    ...data,
                    robotId: id,
                    backtestId: this.#id
                }))
    ...
    #savePositions = (id: string) => {
        const robot = this.#robots[id];
        robot.instance.robotState.positions.forEach((pos) => {
            robot.data.positions[pos.id] = { ...pos, backtestId: this.#id, robotId: id };
        });
    ...
    async handleCandle(candle: Candle) {
        ...
        for (const id of robots) {
            ...
            await robot.instance.check();

            this.#saveSignals(id);
            this.#savePositions(id);

            await robot.instance.run();
    ...
    finish(cancel = false) {
        ...
        if (this.isProcessed) {
            //   if (this.settings.populateHistory) this.#robotState = this.#robots[this.#robotId]?.instance?.ro;
