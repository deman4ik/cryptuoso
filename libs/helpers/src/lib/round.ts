function numberToString (x: number): string { // avoids scientific notation for too large and too small numbers
...
const round = (x: number, numPrecisionDigits: number = 0) => {
    if(!isFinite(x) || !x && x !== 0) return null;
    if(numPrecisionDigits == 0) return Math.round(x);
...
/*  Determine the range to cut  */
    ...
    let precisionStart = afterDot      // 0.(0)001234567
