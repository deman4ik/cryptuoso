...
class ReviewSelectionDialog extends ComponentDialog {
    ...
    constructor() {
        ...
        this.addDialog(
            new WaterfallDialog(WATERFALL_DIALOG, [this.selectionStep.bind(this), this.loopStep.bind(this)])
        );
    ...
    async selectionStep(stepContext: WaterfallStepContext) {
        ...
        (stepContext as any).values[this.companiesSelected] = list;
        ...
        const options =
            list.length > 0
                ? this.companyOptions.filter(function (item) {
                      return item !== list[0];
                  })
    ...
    async loopStep(stepContext: WaterfallStepContext) {
        ...
        const list = (stepContext as any).values[this.companiesSelected];
...
export { ReviewSelectionDialog, REVIEW_SELECTION_DIALOG };
